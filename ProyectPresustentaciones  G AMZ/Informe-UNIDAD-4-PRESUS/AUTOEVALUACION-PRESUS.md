...
| E2 | Caché externa | El `docker-compose.yml` incluye Redis pero el backend **no tiene** `spring-boot-starter-data-redis` en el `pom.xml` ni anotaciones `@Cacheable`. No hay caché implementada. | **Falta** | Criterio de verificación: `redis-cli KEYS "*api_externa*"` falla. | 🔴 Alta |
